...
var addr = flag.String("listen", ":7777", "addr to listen")
...
func main() {
		...
		for {
			...
			case <-ticker.C:
				...
				if len(connections) == 0 {
					log.Println("no active conn")
					mutex.Unlock()
					break
				}
				for k := range connections {
					log.Println("active conn:", k)
				}
				mutex.Unlock()
			case <-done:
				log.Println("sever done")
				break OUTER
			}
		}
	}()
	...
	case sig := <-sig:
		close(done)
		log.Println("sever done trigger")
...
func sendHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err := wsutil.WriteServerMessage(conn, ws.OpText, []byte(payload.Text)); err != nil {
		log.Println("send error:", payload.Text, err)
	}
}
...
func wsHandler(w http.ResponseWriter, r *http.Request) {
	...
	log.Println("connection upgraded", conn.LocalAddr())
	defer conn.Close()
	mutex.Lock()
	connections[trackID] = conn
	mutex.Unlock()
	...
	for {
		...
		case ws.OpClose:
			log.Println("close")
			mutex.Lock()
			delete(connections, trackID)
			mutex.Unlock()
			utf8Fin = true

		case ws.OpContinuation:
			log.Println("OpContinuation")
			if textPending {
